...
vim.keymap.set("n", "<CR>", function()
  ...
  -- 1. Check auf Markdown-Format: [Titel](asset:Datei.pdf)
  local m_before = line:sub(1, col):match("%(([^%(%)]*)$") or ""
  local m_after = line:sub(col + 1):match("([^%(%)]*)%)") or ""

  if m_before ~= "" or m_after ~= "" then
    full_link = m_before .. m_after
  else
    -- 2. Check auf Wiki-Format: [[Datei]]
    local w_before = line:sub(1, col):match(".*%[%[(.*)$") or ""
    local w_after = line:sub(col + 1):match("^(.-)%]%].*") or ""
    full_link = w_before .. w_after
  end
  ...
  if full_link ~= "" then
    ...
    local is_no_markdown = clean_target:match("%.%w+$") and not clean_target:match("%.md$")

    if is_asset or is_no_markdown then
      --local find_cmd = "find " .. zettel_root .. "/90_assets -name '" .. clean_target .. "' | head -n 1"
      local find_cmd = string.format("fd -L -F '%s' %s/90_assets | head -n 1", clean_target, zettel_root)
      ...
      if asset_path ~= "" then
        ...
        -- DEBUG ZEILEN:
        print("Suche nach: '" .. clean_target .. "'")
        print("Befehl: " .. find_cmd)

        local asset_path = vim.fn.system(find_cmd):gsub("\n", "")
        print("Gefunden: '" .. asset_path .. "'")
...
vim.keymap.set("n", "<leader>za", function()
  ...
  fzf.files({
    cwd = zettel_root .. "/90_assets/ibm-onedrive/documents",
    prompt = "Link Asset> ",
    ...
    actions = {
      ["default"] = function(sel)
        ...
        --local file_path = sel[1]:match("([^\t]+)")
        --if not file_path then return end
        local file_path = sel[1]:match("([^\t]+)"):gsub("%s+$", "")

        local file_name = file_path:match("([^/]+)$") or file_path
        --local current_file = vim.api.nvim_buf_get_name(0)
        --local relative_to_root = current_file:sub(#zettel_root + 1)
        --local _, depth = relative_to_root:gsub("/", "")
        --local ups = ""
        --for i = 1, depth - 1 do
          --ups = ups .. "../"
        --end

        -- 2. Bild-Erkennung (Präfix "!" hinzufügen)
        --local is_image = (ext == ".png" or ext == ".jpg" or ext == ".jpeg" or ext == ".gif")
        --local prefix = is_image and "!" or ""
        local ext = file_path:lower():match("^.+(%.%w+)$")
        local prefix = (ext == ".png" or ext == ".jpg" or ext == ".jpeg" or ext == ".gif") and "!" or ""

        -- 3. Link formatieren und einfügen
        -- Wir nehmen nur den Dateinamen für die Anzeige []
        --local display_name = file_path:match("([^/]+)$") or file_path
        --local markdown_link = string.format("%s[%s](%s90_assets/ibm-onedrive/%s)",
          --prefix, display_name, ups, file_path)
        --local markdown_link = string.format("%s[%s](asset:%s)",
          --prefix, file_name, file_name)
        local markdown_link = string.format("[[asset:%s]]", file_name)

        vim.api.nvim_put({ markdown_link }, "c", true, true)
      end
    }
  })
end, { desc = "Search Assets & Insert Smart Link" })
